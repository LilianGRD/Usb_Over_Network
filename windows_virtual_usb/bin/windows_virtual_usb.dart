...
Future<void> main(List<String> args) async {
  try {
    var parser = ArgParser()
      ..addOption('atsign',
          abbr: 'a', mandatory: true, help: 'The atSign of the Windows host')
      ..addOption('mac-atsign',
          abbr: 'm',
          mandatory: true,
    ...
    var storageDir = Directory.systemTemp.createTempSync('win_usb_$sessionId').path;
    ...
    var sshnpd = await Sshnpd.fromCommandLineArgs(daemonArgs, atClient: atClient, version: '1.0.0');
    await sshnpd.init();
    await sshnpd.run(); // Starts listening to notification requests from Mac Npt client
    ...
    print('✅ Hub Listening on ${hubSocket.address.address}:${hubSocket.port} for proxy traffic');

    hubSocket.listen((client) {
      print('🔗 TCP connection established from Virtual Port (via Rendezvous tunnel)...');
      client.listen((data) {
